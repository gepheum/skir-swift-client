...
extension Internal {
  ...
  public final class StructAdapter<Frozen, Mutable>: TypeAdapter {
    ...
    private func fromReadableJson(_ obj: [String: Any], target: Mutable, keepUnrecognized: Bool)
      throws
    {
      for (key, val) in obj {
        if let idx = nameToIndex[key] {
          try orderedEntries[idx].fieldFromJson(
            val, target: target, keepUnrecognized: keepUnrecognized)
        }
      }
    }
    ...
    public func decode(_ input: inout [UInt8], keepUnrecognizedValues: Bool) throws -> Frozen {
      ...
      for i in 0..<slotsToFill {
        if let idx = slotToIndex[i] {
          try orderedEntries[idx].decodeField(
            &input, target: t, keepUnrecognized: keepUnrecognizedValues)
        } else {
          try skipValue(&input)
        }
      }
