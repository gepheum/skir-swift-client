...
extension Internal {
  ...
  public final class EnumAdapter<T>: TypeAdapter {
    ...
    public func addConstantVariant(
      ...
      descVariants.append(
        .constant(Reflection.EnumConstantVariant(name: name, number: number, doc: doc)))
    }
    ...
    private func setKindOrdinalEntry(_ kindOrdinal: Int, _ entry: AnyVariantEntry<T>) {
      if kindOrdinal >= kindOrdinalToEntry.count {
        kindOrdinalToEntry.append(
          contentsOf: Array(
            repeating: nil as AnyVariantEntry<T>?, count: kindOrdinal + 1 - kindOrdinalToEntry.count
          ))
      }
    ...
    public func fromJson(_ json: Any, keepUnrecognizedValues: Bool) throws -> T {
      if let num = json as? NSNumber {
        if let i64 = num as? Int64 {
          return resolveConstantLookup(
            Int32(i64), keepUnrecognized: keepUnrecognizedValues, rawJson: json)
        } else {
          let i = num.intValue
          return resolveConstantLookup(
            Int32(i), keepUnrecognized: keepUnrecognizedValues, rawJson: json)
        }
      }
        ...
        case nil:
          if keepUnrecognizedValues {
            ...
            return wrapUnrecognized(
              UnrecognizedVariantData(format: .denseJson, number: num, value: Array(bytes)))
          }
          return defaultValue
        case .removed:
          return defaultValue
    ...
    private func resolveConstantLookup(_ number: Int32, keepUnrecognized: Bool, rawJson: Any) -> T {
      ...
      case nil:
        if keepUnrecognized {
          let bytes =
            (try? JSONSerialization.data(withJSONObject: rawJson, options: [.fragmentsAllowed])).map
          { Array($0) } ?? []
          return wrapUnrecognized(
            UnrecognizedVariantData(format: .denseJson, number: number, value: bytes))
        }
        return defaultValue
      case .removed:
        return defaultValue
